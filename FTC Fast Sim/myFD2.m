...
% fdfin -> when residuals have settled down
...
if(block.IsMajorTimeStep())
    ...
    if(time >= transient_wait)
        ...
        if(norm(r,2)>=a_fd &&  fd_flag == 0 && fd_fin == 0)
            ...
            if(fd_fin==0)
                % Gather information on residuals
                if(fd_flag == 1 && time<fault_time+ test_time_1)
                ...
                % Revert back to no fault diagnosis when residual settles down
                if(norm(r,2)<0.1*a_fd)
                    fd_fin = 0;
                    block.Dwork(5).Data = fd_fin;
                end
            end
